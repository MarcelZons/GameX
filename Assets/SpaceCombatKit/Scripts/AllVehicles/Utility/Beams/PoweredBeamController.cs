...
{
    public class PoweredBeamController : BeamController, IPowerConsumer
    {
        [Header("Power")]
        ...
        public override void SetBeamLevel(float level)
        {
            ...
            if (usePower)
            {
                ...
                float allowedLevel = Mathf.Approximately(maxPowerDrawPerSecond, 0) ? level : allowedPower / (maxPowerDrawPerSecond * Time.deltaTime);
