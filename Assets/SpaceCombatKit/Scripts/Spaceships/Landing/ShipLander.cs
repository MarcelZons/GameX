...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class ShipLander : MonoBehaviour
    {
        ...
        {
            // Call events for the state the ship is in at the start
            if (currentState == ShipLanderState.Launched)
            {
                onShipLaunched.Invoke();
            }
            else if (currentState == ShipLanderState.Landed)
            {
                onShipLanded.Invoke();
            }
        }
        ...
        {
            if (Physics.Raycast(transform.position, -Vector3.up, out hit, minCanLandHeight, groundMask))
            {
                return true;
            }
            else
